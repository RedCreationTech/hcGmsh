...
CAE 只调用 LIMS Facade，不直接访问 C06 Agent。复制 ~.env.example~ 为未入库的
~.env~，通过 ~GMP_LIMS_BASE_URL~ 配置 LIMS API，默认
~http://127.0.0.1:8200~。计算笔记本在 ~192.168.0.121~ 与
~192.168.0.138~ 之间切换时，只需修改 LIMS 的 C06 地址配置，CAE 无需修改。
Markdown 使用说明见 [[file:README.md][README.md]]。
