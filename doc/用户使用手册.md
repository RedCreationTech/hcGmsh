...
13. [已知限制](#13-已知限制)
14. [完整仿真流程与 EXE 能力边界](#14-完整仿真流程与-exe-能力边界)
...
| 区域 | 内容 |
|---|---|
| 顶部 | 菜单栏（File/Model/View/Mesh/Job/Tools/Settings/Help）→ 五个可拖拽图标工具组（项目/编辑/建模/网格/作业）→ 工作上下文条 |
| 工作上下文条 | 模块选择器（Sketch/Part/Property/…/Results 共 13 个模块）+ 项目名 + 当前对象选择器，右侧为随模块切换的快捷按钮 |
| 左栏导航树 | `Model` / `Results` 两个页签：名称过滤、对象/状态双列、右键菜单；Model 页含工作流状态标签与 `Add/Duplicate/Rename/Remove` 动作条 |
| 中栏舞台 | 始终显示的 3D/2D 舞台，三页页签：`Viewport`（3D 视图）、`Plot`（曲线预览）、`Table`（表格预览）；左缘有垂直工具栏（选择/视图/显示/剖切等） |
| 浮动工作窗 | Mesh / Job / Visualization / Results 为独立非模态单实例浮动工作窗（View 菜单可切换；关闭不停止作业、不卸载舞台结果；Results 支持"新建对比窗口"多实例）；其余模块页（Part/Material/Section/Assembly/Step/Interaction/Load/Sketch/Property）在浮动 Module Workspace |
...
| Save Project | `Ctrl+S` | 保存（无路径时弹另存对话框）；成功后右上角短暂显示确认与文件名 |
| Save Project As... | `Ctrl+Shift+S` | 另存为；使用带蓝色加号徽标的独立图标，成功提示明确标注“另存为” |
...
| 菜单项 | 快捷键 | 说明 |
|---|---|---|
| Undo | `Cmd+Z`（macOS）/ `Ctrl+Z` | 撤销草图编辑；浮动 Sketch Editor 获得焦点时同样生效 |
| Redo | 平台标准重做组合键 | 重做草图编辑；macOS 通常为 `Cmd+Shift+Z` |
...
同一根节点下的对象名称必须唯一。新增或重命名时若名称为空或与已有对象相同，弹窗会保留并显示错误，可直接修改后再次确认；复制及系统自动创建的对象会追加唯一后缀。不同根节点之间可以使用相同名称。
...
- **右键菜单**：根节点上为 `Add <根名>`（Mesh/Jobs/Results 根另有打开对应工作窗）；子节点上为 `Edit Properties...` + `Add / Duplicate / Rename / Remove`；
- **选中节点**：单击只选择节点；双击（或按 `Enter`、右键菜单 `Edit Properties...`）打开该节点的模态浮动属性表单（详见 [§9](#9-propertyeditor-属性编辑器)）；Module Workspace 中的 Property 模块页提供选中节点的只读上下文浏览。
...
- 新建或双击 Sketch 后进入二维编辑；草图选择支持单击单选、`Shift+单击`多选，Delete 工具或删除键用于删除图元。
- 舞台左侧的四向箭头在 Sketch 模块中表示“移动图形”：默认在矩形任意边上按下会选中并移动整个矩形，圆、圆弧、独立直线同理；按住 `Option/Alt` 可下钻到矩形单边等子图元。
- `Shift+单击`用于多选完整图形，`Shift+Option/Alt+单击`用于多选子图元；切到移动工具后，拖动任一已选成员即可整体移动当前选择集合。
- 当前绘制工具会持续以蓝底白字高亮，并在工具区下方显示“当前工具”；完成一次绘制后工具仍保持生效，直到主动切换。
- Sketch Editor 使用独立的紧凑双栏布局和窗口尺寸记忆；其大小不会沿用 Part、Mesh 等工作窗，底部“完成编辑”保持为定宽主按钮。
- 整体视图平移使用鼠标中键拖动；缩放可使用滚轮或左侧缩放按钮，适配窗口不改变草图几何。
- 完成编辑后舞台保留只读预览；可继续选择、清除、中键平移、缩放和适配窗口。点击移动图形、绘制或删除等修改工具时，会自动打开当前草图的编辑会话并保留所选工具，不会无提示地退回点选。
- Undo/Redo 按钮和 Tools 菜单快捷键以一次草图修改为一步，在浮动编辑窗获得焦点时仍可使用。
- 删除当前预览草图会同步关闭二维预览；若该草图已被拉伸等 Feature/Part 引用，当前 3D 网格会立即从舞台清除，避免继续显示已经失去来源的旧结果。
...
- 删除 Part 会同步删除属于它的 Feature 历史并清空当前 3D 舞台；删除 Feature 也会解除所属 Part 的几何结果引用。该操作不会物理删除磁盘上已经生成的 BREP/MESH 文件，后续仍可按需重新载入。
- 一个 Part 可以包含同一次特征生成的多个独立 Volume。例如同一草图中的两个相离闭合轮廓沿 Z 轴拉伸后仍属于一个 Part；切换到 Sketch 再返回 Part 时会恢复该 Part 的完整 MESH，不会只显示第一个 Volume。

### 5.2 Property 模块

![Property 模块](images/module_01_Property.png)

双击模型树节点打开模态浮动属性表单（即 PropertyEditor，详见 [§9](#9-propertyeditor-属性编辑器)）：编辑在表单内的编辑缓冲中进行，"确定"经校验后一次性写回节点，"取消"（或 `Esc`/关闭窗口）丢弃修改；Module Workspace 中的 Property 模块页提供当前选中节点的只读上下文浏览。
...
- 创建后在模型树中选中新节点，双击打开属性表单编辑参数（密度、导热系数、弹性常数等，见 §9 的模板预设）。
...
- `Open Assembly Root` 打开独立的 Assembly 根节点，`Create Instance` 新建 Part 实例；
- 实例表单可选择 Part，并配置平移、按 X → Y → Z 应用的角度制旋转、三轴缩放、可见性和顺序；
- `Build Assembly` 将可见实例的变换落实到 Gmsh，并为每个实例生成 `<实例名>` 顶维物理组和
  `<实例名>_surface` 边界组；后续网格生成会先重建装配，避免使用过期变换；
- 修改源 Part 或实例参数后，Assembly 及下游 Mesh、输入和 Job 会标记为过期。项目保存重开后
  Part 引用、变换参数和活动装配模型来源保持不变。
...
Gmsh 面板位于专用的 Mesh Workspace 浮动窗口（可从模块选择器、舞台左栏按钮或模型树右键菜单打开，单实例），功能与参数见 [§6](#6-mesh-模块gmshpanel参数详解)。
...
Job Workspace 浮动窗口含两个页签："作业列表"（LIMS 式作业监控：状态筛选、九列作业表、右侧详情面板，详见 [§7.1](#71-作业管理)）与"MOOSE 设置"（本地作业配置，详见 [§7.2](#72-moose-panel-参数详解)）。
命令条按钮：
...
- 按钮：`Open Results Root`、`Refresh List`、`Import Result File...`（导入外部结果文件）、`Preview`（展开/折叠预览区，默认折叠）、`Open in Viewer`（Exodus 或网格加载）、`Open as Text`（文本预览，超过 8KB 截断）、`New Comparison Window`（新建对比窗口）；
- 预览区（点 `Preview` 展开）显示：路径、所属 job、文件大小/修改时间、类型与前 6 行内容；
...
GmshPanel 位于专用 Mesh Workspace 浮动窗口，按"模型 / 几何 / 分组与网格场 / 网格"四个页签组织（运行日志已统一到主窗口底部 Console，带时间戳与分类）。分组如下：
...
Job Workspace 的"作业列表"页签为 LIMS 式作业监控，本地作业与远程（LIMS）作业并列显示：

- **作业表** 9 列：`Name / Status / Case / Progress / Start / Duration / Type / Exec / Result`；
- **操作行**：`Run`、`Stop`、`Retry`（重跑）、`Open Log`（弹窗显示完整日志）、`Open Result`（把该行结果载入视口）；`State` 状态筛选（`All / Queued / Running / Completed / Failed / Canceled`）+ `Refresh` + `Auto (5s)` 自动刷新；
- **右侧详情面板**：选中行显示进度条与进度文本、**Execution Details** 执行状态（Input/PID/Parallel/CPU/Memory/Step/dt/Phy. Time/Converged/Elapsed/ETA/Heartbeat/Health 等），以及 **Artifacts** 制品列表（`Refresh Files` 刷新、`Download Selected` 下载选中制品）；本地作业另附只读详情文本；
- 本地作业状态流转：`Running` → `Completed / Failed`（按退出码），自动记录起止时间与时长；远程（LIMS）作业支持状态刷新与 `Cancel` 远程取消，产出的 Exodus 结果自动登记 Results 节点。

### 7.2 MOOSE Panel 参数详解

"MOOSE 设置"页签内分"算例设置 / 执行配置 / 输入文件"三个子页（运行日志已统一到主窗口底部 Console，带时间戳与分类），参数分组如下：
...
- 动作行 `Run / Check Input / Stop`；运行日志实时镜像到主窗口底部 Console（自动识别产出的 `.e` 文件并通知视口）。
...
双击模型树任意节点（或按 `Enter`、右键菜单 `Edit Properties...`）打开模态浮动属性表单，标题为 `编辑«Kind» — «名称»`，含四个页签。表单内维护独立的编辑缓冲："确定"经校验（名称唯一性、类型参数规则）后一次性写回节点并标脏项目；"取消"、`Esc` 或关闭窗口则丢弃全部修改，不改变项目脏状态。Module Workspace 中的 Property 模块页仍提供选中节点的只读上下文浏览。
...
- **校验**：缺失必填字段时显示红色 `Missing required fields: …`。典型规则：Transient 步要求 `dt>0, end_time>0`；FunctionDirichletBC 要求 `function`；BodyForce 要求 `value` 或 `function`。

### Validation

**Validation Summary** 汇总全树校验结果：`Node / Issues` 表，过滤器 `Current Type Only` / `Only With Issues`（默认勾选），`Refresh` 刷新，`Go To Node` 跳转到问题节点。
...
| `GMP_TOUR_STEP_FILTER` | 仅执行名称包含指定文本的巡览步骤，用于定向回归；需与 `GMP_SCREENSHOT_DIR` 配合 |
...
本章回答一个关键问题：**打包的 Windows EXE（`gmp_ise-windows-x64.zip`）覆盖了"预处理 → 有限元计算 → 后处理"中的哪些部分？客户真正用起来时，完整流程长什么样？**

### 14.1 三部分能力边界（速查表）

| 环节 | 内容 | EXE 是否内置 | 说明 |
|---|---|---|---|
| **预处理** | CAD 导入（STEP/IGES/BREP）、几何基元/变换/布尔、物理组、网格尺寸场、2D/3D 网格算法、网格质量报告 | ✅ 完整内置 | Gmsh C++ API 以**进程内**方式链接进 EXE，无需任何外部程序 |
| **模型定义** | 材料/截面/分析步/边界/载荷/函数/变量/输出，模板预设与校验 | ✅ 完整内置 | 模型树 + PropertyEditor |
| **输入生成** | 模型树 → MOOSE `.i` 文件，模板管理，网格块注入 | ✅ 完整内置 | `Sync Model to Input` + `Write Input` |
| **作业编排** | 作业表、运行控制、日志监控、结果自动登记 | ✅ 完整内置 | 但启动求解依赖外部 MOOSE 可执行文件 |
| **有限元计算** | MOOSE 求解（PETSc/MPI 并行） | ❌ **不包含** | 需在远程 Linux 机器或本机 WSL2 单独部署（见 14.3） |
| **后处理** | ExodusII (`.e`) 加载，标量/向量/变形/切片/探针/时间序列/Plot/Table，网格预览与拾取 | ✅ 完整内置 | VTK 9.6 以进程内方式链接进 EXE |

一句话：**EXE = 完整的前后处理器 + 求解调度器；求解器本体按设计独立部署**（这也是项目的既定架构——计算资源与桌面环境分离，求解机可以是大内存 Linux 服务器）。
...
| 方式 | 适用场景 | 配置要点 |
|---|---|---|
| **A. 远程 Linux 求解机（推荐）** | 有独立的 Linux 服务器/工作站 | 在求解机上按官方流程安装 MOOSE：`conda create -n moose -c https://conda.software.inl.gov/public -c conda-forge moose-dev`，再源码构建 `moose_test-opt`（或热力耦合用 `modules/combined/combined-opt`）。EXE 侧通过文件交换协作（见 14.4 第 5 步） |
| **B. 本机 WSL2** | 只有一台 Windows 机器，算力需求不大 | 在 WSL2 (Ubuntu) 内同法安装 MOOSE；EXE 的 `Runner` 选 `WSL`，直接调度 `wsl.exe` 运行（构建时需启用 `GMP_ENABLE_WSL_RUNNER`，见打包脚本注释） |
| **C. 本机 MOOSE** | Windows 原生 MOOSE（官方不支持，自行维护） | 不推荐；MOOSE 官方不支持 Windows 原生构建 |

> `Remote`（SSH 直连调度）在 UI 中预留，当前版本为占位实现，落地后方式 A 可以免拷文件。

### 14.3 文件与数据流向

```
[EXE] 几何/网格 ──► out/*.msh ──┐
[EXE] 模型树  ──► out/*.i   ──┤  拷贝/共享目录
                               ▼
                    [求解机] mpiexec moose_test-opt -i case.i
                               │
                               ▼
                    out/*.e (ExodusII)
                               │
[EXE] 后处理 ◄── 拷回/共享 ◄──┘
[EXE] 项目快照 *.gmp.yaml（模型树 + 全部面板设置）
```

### 14.4 客户完整使用流程（端到端）

**第 0 步｜一次性准备**：按 14.2 选定求解方式并部署好 MOOSE。

**第 1 步｜几何与网格（EXE, Mesh 模块）**
- `Open Geometry` 导入 STEP/IGES/BREP，或用 Primitives 建基元；
- 在 `Physical Groups` 组创建边界/体组（**名称会用作 MOOSE 的 boundary/block 名**，如 `fixed_end / load_end / beam`）；
- 设置网格尺寸、算法、Recombine 等 → `Generate 3D Mesh` → 核对日志中的单元数与质量；
- 网格自动写入模型树 `Mesh` 节点并预览到视口。

**第 2 步｜物理定义（EXE, Property/Material/Section/Step/Load/BC 模块）**
- 建 Variables（如 `disp_x disp_y disp_z` / `T`）；
- Materials：用模板预设（`Linear Elastic (isotropic)` 等）填弹性常数；
- Steps：Transient/Steady 及时间步参数；
- BC/Loads：在属性表单参数页签的 `Groups` 框把边界/载荷绑定到第 1 步建的物理组上；
- 看左栏 **Workflow status** 全部变 `ready`，**Validation Summary** 无缺失项。

**第 3 步｜生成输入文件（EXE, Job 模块）**
- `Sync Model to Input`（Ctrl+Shift+R）→ 在 Input Editor 检查；
- ⚠️ 已知问题：含 TensorMechanics 时删除多余的 `type = TensorMechanics` 行（见 §11.2 注意）；
- 选 FileMesh 模板或 `Insert Mesh Block` 注入 `.msh` → `Write Input`。

**第 4 步｜提交求解（按部署方式）**
- **方式 A（远程机）**：把 `case.i` 与 `box.msh` 拷贝到求解机，执行
  `mpiexec -n <核数> moose_test-opt -i case.i`；
- **方式 B（WSL2）**：EXE 的 Job 模块填好 Executable（WSL 内路径）、Input File、Mesh File，`Runner = WSL`，点 `Run`；
- 求解产出 `case_out.e`（ExodusII）。

**第 5 步｜后处理（EXE）**
- 方式 A：把 `.e` 拷回本机；方式 B：直接可见；
- `Results` 模块刷新列表 → 双击载入；`Visualization` 做标量云图/变形放大/切片/探针/时间序列；`Plot`/`Table` 出数据；`Save Screenshot` 出图。

**第 6 步｜归档**
- `Save Project` 存 `*.gmp.yaml`（下次打开完整恢复模型树与全部面板状态）；需要技术支持时用 `Export Debug Bundle` 打包项目+网格+输入+日志。

### 14.5 "一键提交"按钮的适用条件

`Generate & Submit` / `Submit (Mesh + Sync + Run)` 实现的是"网格 → 同步 → 运行"全链路自动化，但**最后一环 Run 仍要求本机可找到 MOOSE 可执行文件**（`Executable` 已配置、环境变量 `GMP_MOOSE_EXEC`、或自动探测命中，见 §12）。在只装了 EXE 的客户机上：

- 方式 B（WSL2 已配 MOOSE）：一键流程可用；
- 方式 A（远程机）：一键流程会在 Run 环节因找不到可执行文件而失败，请改用 14.4 第 4 步的文件交换方式。

### 14.6 给客户的一句话总结

- 只装 EXE：**建模、网格、材料/边界/载荷定义、输入文件生成、结果可视化全部可用**；
- 真正出计算结果：需要一台按 14.2 配置好的 MOOSE 求解机（远程 Linux 或本机 WSL2），`.i + .msh` 送过去，`.e` 拿回来。
