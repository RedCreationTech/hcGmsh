...
4. 观察进度窗阶段、百分比/已用时以及主窗口重绘；在一次生成中点击“取消”。
5. 再次生成并等待完成，检查中央舞台和日志；随后不重建部件，直接再次生成一次。

通过标准：过细尺寸在剖分前被快速拒绝，提示可读且按钮立即恢复，原有效输出不被覆盖；正常生成时进度窗持续更新、窗口不呈系统“未响应”、取消可终止且不损坏当前 OCC 部件；成功后舞台显示真实 3D 体网格，日志含 3D 单元与质量统计且不再出现 element type 1 质量错误；同一部件可连续生成，不退化为 12 节点空网格。

## 网格生成缺陷回归记录

| 用例 | 结果 | 备注 |
|---|---|---|
| TEST-P4-GEN-04 | 待人工验证 | 自动回归已覆盖生成、舞台、模型恢复、超限拒绝；人工确认进度窗与取消手感 |
