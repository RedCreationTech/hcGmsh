...
### 增量迭代 3（2026-08-12）：中央舞台跟随页签切换

**问题**：切换"草图"/"部件"页签时右侧属性栏跟随、中央视口不跟随——草图页签里仍显示 3D 网格（2D 草图与 3D 内容叠显/遗留）。

**修复**：
- VtkViewer：进入草图会话时隐藏 3D 场景 actor（主网格/节点/外框/色标/选中高亮）并暂存可见性，退出会话时逐项恢复——2D/3D 不再叠显；
- MainWindow：页签切换钩子——进入"草图"页签时若未在编辑，自动打开当前选中（或首个）草图进入 2D 编辑；离开"草图"页签时自动保存并退出编辑会话、恢复 3D 场景。
- 坑：页签判断必须用构造期解析的索引（`sketch_tab`），不能用 `tabText` 比英文——中文界面下文本已被翻译。

**自测**（QTest 驱动真实 MainWindow）：画矩形 → 完成编辑 → 载入 box.msh → 切草图页签（自动进入 2D 编辑、3D 隐藏，截图确认纯净 2D 草图）→ 切部件页签（会话关闭、3D 网格恢复显示）→ 再切回（自动重进编辑）—— ALL PASS。

### 增量迭代 4（2026-08-12）：多闭环特征 + 模型树联动

| 项 | 内容 | 状态 |
|---|---|---|
| 多闭环轮廓 | 特征操作作用于草图**全部闭环**：相离环各自成体（两个圆→两个圆柱），嵌套环按奇偶层作内孔（矩形+内圆→带孔实体，1 个体积）；多结果合并为 OCC Compound 导入 gmsh。覆盖拉伸/旋转/扫掠；放样截面仍为单闭环（配对规则后续版本） | ✅ |
| 模型树联动 | 点击模型树节点即切换模块页签 + 右侧栏 + 中央舞台（Sketches→草图、Parts/Features/Datums→部件、BC/Variables/Functions/Outputs→属性等）；点草图子节点直接进入该草图的 2D 编辑（已在草图页签时直接切换编辑对象）；新建草图自动跳转并进入编辑 | ✅ |

**踩坑记录**
- 环包含判定不能用**质心**（矩形质心恰为内孔圆心 → 双向包含误判，外/孔全丢失）；改用各环**边界采样点**做射线法。
- 运行期页签查找不能用 `tabText`/`module_tab_index`（中文界面文本已翻译）；全部用构造期缓存的索引。

**自测**：多闭环独立 harness（2 圆→2 体积、矩形+圆孔→1 带孔体积、2 圆旋转 90°→2 体积）ALL PASS；树联动 harness（点 sketch_2 →切草图页签且编辑的就是 sketch_2、点 Parts→切部件页签且会话保存关闭）ALL PASS；回归 harness2/3/4（撤销重做/特征链路/舞台切换）ALL PASS；13 模块巡览无回归。
...
|---|---|---|
| 线段（两点） | ✅ | 端点吸附；坐标输入框绘制 ❌ |
| 矩形 | ✅ | 两对角点，自动生成 4 线+重合约束（需求工具栏未列但已交付） |
| 圆 | ✅ | 圆心+半径点；参数框输入 ❌ |
| 圆弧 | 🟡 | 已支持"圆心-起点-终点角"；"三点圆弧"❌、"切线圆弧"❌ |
| 多段线 | ❌ | 连续折线，双击/Enter 结束；坐标表批量输入 ❌ |
| 椭圆 | ❌ | 需扩展 SketchDocument 图元类型 |
| 样条曲线 | ❌ | 控制点序列 + 阶次/拟合/开闭；需扩展图元类型 |
| 点 | ❌ | 独立点图元（基准/参考用） |
| 构造线 | ❌ | 辅助线图元（不参与闭环/特征），需在图元上加 construction 标志 |
...
| 需求项 | 状态 | 说明 |
|---|---|---|
| 重合/水平/垂直/平行/垂直(Perpendicular) | ✅ | 求解器+UI 均已支持 |
| 等长/等半径 | 🟡 | 求解器已映射，UI 未暴露按钮 |
| 相切/同心/对称/相交（点在线上） | ❌ | planegcs 有能力，需映射+UI |
| 尺寸约束（距离/半径） | ✅ | driving 驱动尺寸 |
| 角度/直径尺寸 | 🟡 | 求解器已映射 Angle，UI 未暴露；直径 ❌ |
| 修改尺寸（改值重求解） | 🟡 | 求解器 set_driving_value 已支持，UI 无双击改值入口 |
| 参考尺寸（只读测量） | 🟡 | 契约 driving=false 已留，UI/显示 ❌ |
| 参数管理器 | ❌ | 命名参数驱动尺寸 |

**草图工具栏（需求 1.3 逐条）**：撤销✅ 重做✅ 添加尺寸✅ 添加约束✅ ｜ 修改尺寸❌ 参数管理器❌ 阵列❌ 移动/复制❌ 修剪❌ 偏移❌ 投影❌ 曲线变构造线❌ 两点构造线❌ 样条❌ 倒角（2D圆角/倒角）❌ 三点弧❌ 切点弧❌ 椭圆❌ 多段线❌ 点❌ ｜ 保存/打开草图：项目内持久化✅，独立草图文件互导 ❌（第 9 章 TODO）
...
|---|---|---|
| 创建部件 | 🟡 | 选草图建部件 ✅；维度/类型选择 ❌ |
| 拉伸 | 🟡 | 基本拉伸 ✅（多闭环/自动网格显示）；对称拉伸/拔模角/自定义方向 ❌；实时预览 ❌ |
| 旋转 | 🟡 | 绕固定 Y 轴 ✅；自定义旋转轴/对称 ❌ |
| 扫掠 | ✅(基本) | 轮廓沿路径 ✅；截面方向/扭转控制 ❌ |
| 放样 | 🟡 | 两截面实体/壳 ✅；多截面 ❌、匹配点/导向线 ❌ |
| 切除 | ❌ | OCC BRepAlgoAPI_Cut，需求核心项 |
| 圆角/倒角（3D） | ❌ | BRepFilletAPI，边/顶点拾取 + 半径/距离 |
| 创建壳 | ❌ | BRepOffsetAPI_MakeThickSolid |
| 镜像（3D） | ❌ | BRepBuilderAPI_Transform 镜面 |
| 编辑特征 | ❌ | 需参数化重建引擎（改参数→重放特征树） |
| 禁用特征 | ❌ | 依赖特征树父子关系与重建 |
| 删除特征 | 🟡 | 树节点可删（仅删记录，无几何联动） |
| 拆分边/面/体 | ❌ | 需求含多种拆分方式，OCC 工作量中-大 |
| 基准面/轴/点/局部坐标系 | ❌ | Datums 根节点已建，无创建功能（方案 Q10：本期缓办） |
| 几何导入/导出 | 🟡 | 特征自动落盘 brep ✅；STEP/BREP 主动导入导出 ❌（OCC 读写器现成） |
| 部件管理器 | 🟡 | 部件清单（树+模块列表）即简易版 |

### 11.3 建议的下一迭代优先级

- **P0（高频基础，独立小项）**：多段线、点、三点圆弧、椭圆、移动/复制、修剪、偏移、2D 镜像/阵列；修改尺寸（双击尺寸值重求解）、角度/直径尺寸 UI、等长/相切/同心/对称约束 UI；**切除 Cut**、**圆角/倒角**。
- **P1（进阶，依赖基础设施）**：构造线、样条曲线、放样多截面、对称拉伸/拔模角、自定义旋转轴、创建壳、3D 镜像、STEP/BREP 导入导出。
- **P2（重投入或已定缓办）**：编辑/禁用特征（需参数化重建引擎）、拆分边/面/体、基准面/轴/点/局部坐标系（Q10 已定缓办）、参数管理器、草图外部格式互认（第 9 章 TODO）。
