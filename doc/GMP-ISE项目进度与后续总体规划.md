...
> 当前代码基线：`main@4c8a215e9f8c`
> 规划模式：项目总览 + 开发任务编排
> 主要依据：`doc/UI重构开发任务清单.md`、`doc/UI重构下一阶段任务清单.md`、`manual/test05.md`、`doc/v0.2重构任务清单.md`、`doc/v0.2.1架构硬化任务清单.md`、`doc/七个CDP标准算例CAE人工复现能力核对与任务计划.md`、`doc/ref/HC-Gmsh-v0.2-重构成果复核与进一步优化计划.md`
> 状态：**已批准执行；G1 已关闭，HARD-010～050 已完成；7 个 CDP 标准算例 UI 人工复现被指定为下一里程碑，G2 产品实现顺延**
...
即执行 `TASK-HARD-010～050` 后停止架构扩张。对象级 DependencyGraph（HARD-060）和内部 Undo/Redo（HARD-070）保留为后续任务，不阻塞 STD-CAE 与后续 G2/G3。
...
4. 在新增高级功能前，先完成 7 个已冻结 CDP 标准算例的 CAE UI 人工组成复现：V01 结构化首例闭环 → V02/R02/G33 → G32/G34 → R01；已有 `.i` 导入作为辅助入口，不替代建模能力。
5. 恢复 Phase 5：Reference Point/Coupling（G2）；已完成的目标语法探针保持冻结，不重复提交。
6. 在多 Step 前完成对象级依赖图，再实施 Phase 6（G4）。
7. 工业 CAE 闭环稳定后，再进入 v0.3 Feature Graph/参数化 CAD。
...
`HARD-010～050` 已解决持久 ObjectId、Document 真源、Store 直连 Document 和最小可逆领域命令问题。Reference Point/Coupling 已具备实现条件，但按产品顺序排在 STD-CAE 之后；后续功能必须继续遵守：
...
- `HARD-060`、`HARD-070` 分别保留到 G4 与 Feature Graph 前，不重新阻塞 STD-CAE 或 G2/G3。
...
| 项目 | 当前状态 | 结论 |
|---|---|---|
| Git 基线 | `main`、`origin/main` 均为 `4c8a215e9f8c` | V1 标准算例闭环与 029～040 修复已推送 |
...
| CDP 标准算例 | STD-CAE | ✅ V01 人工闭环完成 | V01 人工操作已于 2026-09-21 完成，029～040 缺陷修复人工回归于 2026-09-24 全部收口；V1 回归闭环完成，下一个算例为 V02（TASK-STDCAE-050），当前 1/7 达到 D |
| 高级约束 | Phase 5 / G2 | 🟡 原型冻结/顺延 | 目标语法与最小传力探针已验证；产品领域对象、UI、生成、校验和端到端准出待 STD-CAE 后继续 |
| 高级材料/基准 | Phase 5 / G3 | 🟡 基准已选/CAE 闭环待实现 | 7 个标准算例及其冻结证据已由 damASR 提供；先完成 STD-CAE UI 人工复现，再决定是否需要额外 M-02/M-03 |
| 多阶段 | Phase 6 / G4 | ⚪ 仅有需求 | 当前只保存多个 Step，生成器只使用首个 Step 并告警 |
| 架构 | v0.2 Stage 0～7 | ✅ 已关闭 | 7 个 Stage 已完成；RG-D 人工总准出通过；`v0.2-rc1` 已冻结 |
| 架构硬化 | v0.2.1 | ✅ 核心切片完成 | HARD-010～050 已完成；已具备 STD-CAE 与后续 G2/G3 开发条件 |
...
| 能力域 | 已交付 | 尚缺 | 下一动作 |
|---|---|---|---|
| 项目生命周期 | 新建、打开、保存、另存为、项目状态隔离、schema v2、持久 ObjectId、Document 直接持久化与领域写入 | 无当前功能阻塞 | STD-CAE 与后续 G2/G3 继续复用 |
...
| 材料/Section | 线弹性、CDP 表单、Section 指派、单位合同 | 7 个 CDP 标准算例尚不能由 UI 完整复现 | STD-CAE |
...
| MOOSE 输入 | 结构化生成、Custom Blocks、diff/冲突、来源报告、确定性 | Exodus 体组未接入结构化链路；历史输出请求不够细；部分算例专用对象未结构化；已有 `.i` 导入为辅助缺口 | STD-CAE 后再做 G2/G4 |
| Snapshot | contract v2、hash、材料 CSV、网格/输入打包、上传 | Exodus 导入未生成完整 manifest；辅助导入入口尚不能按 `.i` 目录打包 | STD-CAE P0 |
| Job | 本地/远端提交、状态、4 MPI、取消、日志与文件列表基础 | 无当前 G1 遗留 | STD-CAE 与后续 G2/G3 继续复用 |
| Results | Exodus/CSV 登记与 VTK 回放基础、变量/时间轴/切片 | 无当前 G1 遗留 | STD-CAE 与后续 G2/G3 继续复用 |
...
HARD-010～050 已关闭稳定身份与 Tree 真源问题。当前产品 P0 是 STD-CAE：在新增高级功能前，让用户能从空项目开始，经结构化 UI 逐项组成 damASR 的 7 个冻结标准算例并完成计算闭环。

### 3.2 P1：既定产品功能

1. STD-CAE：V01 结构化首例闭环、V02/R02/G33 复用闭环、G32/G34 专用 Physics 闭环、R01 诊断闭环。
2. G2 Reference Point/Coupling。
...
2. 内部 Undo/Redo 栈（HARD-070）：应在 Feature Graph 大量可编辑操作前完成，不阻塞 STD-CAE 或 G2/G3。
...
| 2026-09-17-023 缺装配预览/体网格来源切换 | 已登记 | STD-CAE/G2 使用反馈后处理 |
...
| B. 立即全面补功能 | 短期表面进度最快 | 新增更多 name reference、Tree 写路径和不可逆编辑，随后返工 | 不推荐 |
...
### Milestone M3：STD-CAE 七个 CDP 标准算例 UI 人工复现

- **阶段目标**：不使用终端、不粘贴整份 `.i`，从空项目开始通过 Model Tree 和属性表单逐项组成标准算例，再完成快照、远端 Job、制品下载和 Results 回放。
- **当前结论**：共同 CDP 材料、Function、BC、Step、QuasiStatic 和主要 Outputs 表单已具备；V01 的 Exodus manifest、冻结历史输出预设及端到端结构化生成合同已完成，达到 B。V01 标准算例原流程人工操作已于 2026-09-21 完成（C/D），随后发现的 029～040 修复人工回归于 2026-09-24 全部通过，V1 回归闭环完成，完整闭环计数 1/7。
- **阶段顺序**：V01 结构化首例 → V02/R02/G33 → CDPQuasiStatic/Predictor → G32/G34 → R01；已有 `.i` 导入作为后置辅助入口。
- **任务与验收**：执行 `TASK-STDCAE-010～100`、`TEST-STDCAE-01～12`，详见 [七个 CDP 标准算例 CAE 人工复现能力核对与任务计划](七个CDP标准算例CAE人工复现能力核对与任务计划.md)。
- **人工文档**：已创建 `doc/人工算例操作说明文档.md`，详细给出 V01 从空项目逐项组成、生成核对、提交和结果验收步骤。
- **不做**：不新建通用 `.i` AST/IDE，不重算或覆盖 damASR 冻结基线，不自动提交长耗时作业。
- **状态**：✅ V01 人工闭环完成（2026-09-21 标准算例操作 + 2026-09-24 029～040 回归收口）；下一步 V02（TASK-STDCAE-050）。

### Milestone M4：G2 Reference Point / Coupling
...
### Milestone M5：G3 CDP 与认可基准

#### TASK-PLAN-040 冻结基准和容差

- **优先级**：P0。
- **关联**：TASK-P5-05、M-03。
- **工作**：以 damASR 已冻结的 V01/V02/R01/R02/G32/G33/G34 为参考输入和证据源；保持各例材料、网格、加载、单位、指标、已知限制及各自冻结 solver 身份。
- **准出**：STD-CAE 先完成结构化 UI 组成与计算闭环；不得以“调参到通过”改写冻结标准，已有 `.i` 导入只作对照证据。
- **状态**：✅ 基准集合已由用户指定并完成静态核对；CAE UI 闭环转由 M3 执行。
...
| 轨道 | 可并行内容 | 不可越过的边界 |
|---|---|---|
| 外部验证轨道 | 仅在本地 P0 完成后，由用户人工触发 V01→其余六例的真实 Job | 不自动占用计算节点；不覆盖冻结基线 |
| 本地开发轨道 | TASK-STDCAE-010～040；完成 V01 后再排 TASK-PLAN-031A～031G | V01 结构化首例未关闭前不新增 G2/G4/Feature Graph 功能 |
...
| STD-CAE | 逐块等价合同、Exodus manifest、结构化生成确定性 | 7 个标准算例逐例由 UI 组成、提交并回放结果 | 首例不通过则不扩算其余算例 |
| G2 | 引用/维度/冲突合同 | 真实传力与反力平衡 | 不进入 G4 |
...
| 结构化 UI 只生成近似模板或替换网格 | 结果不能与冻结基线比较 | 共同 `.e` 以 hash 保护；生成 `.i` 按块/type/参数/引用与 damASR 冻结输入做语义比较 |
| CDP 基准定义不清 | 无限调参、结论不可复核 | 已指定 7 个冻结标准算例；沿用各例既有指标、容差与已知限制 |
...
- **推荐**：先完成 STD-CAE 与 G2～G4，再进入 Feature Graph。
- **理由**：G1 已 9/9 关闭；七个冻结 CDP 算例已被指定为下一里程碑，Feature Graph 是更大的新产品阶段。
- **决策**：✅ 已确认（2026-09-20），先完成 STD-CAE 与 G2～G4，再进入 Feature Graph。

### D-02 v0.2.1 是否只包含 HARD-010～050？

- **推荐**：是。HARD-060 放到 G4 前，HARD-070 放到 Feature Graph 前。
- **理由**：二者重要但不阻塞 STD-CAE 或 G2/G3，继续把它们作为功能恢复闸门会延迟用户价值。
...
- **决策**：✅ 已确认（2026-09-20）。采用 damASR `docs/examples/` 下 V01、V02、R01、R02、G32、G33、G34 七个冻结标准算例及各自证据/限制；下一里程碑先做 CAE UI 人工复现。

### D-05 版本命名是否调整？

- **推荐**：暂不改旧文档中的 v0.3/v0.5 名称，以 G1～G4、STD-CAE 和 M0～M8 作为执行顺序；等 STD-CAE 首例关闭后再决定下一个版本承诺。
...
- **CDP 标准算例：基准已冻结；V01 标准算例原流程人工操作已完成（2026-09-21），029～040 缺陷修复回归已收口（2026-09-24）；达到 D 的完整复现当前 1/7，下一例为 V02。**
- **高级约束、多 Step：尚未交付。**
...
4. 7 个冻结 CDP 标准算例均可从空项目通过 CAE 结构化 UI 逐项组成、提交、下载并回放，且逐例具备人工操作文档和验收记录。
5. G2 Reference Point/Coupling 有真实传力证据。
6. G3 结构化 CDP 生成结果在对应冻结标准的比较口径内。
7. G4 多 Step 不丢失材料内部状态且可追溯。
8. 全程保持 schema v2 兼容、真实点击巡览基线只增不减、G1 确定性不退化。
