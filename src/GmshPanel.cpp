...
bool GmshPanel::build_assembly(QString* error) {
  ...
  return false;
#else
  // TASK-V02-040：装配几何构建下沉到 AssemblyGeometryService；面板保留
  // 模型选择器重登记、清单失效与日志/发射编排（019 时序不变：服务内
  // 已完成组恢复，之后才是选择器与 note_external_model_loaded）。
  ensure_gmsh();
  const gmp::AssemblyBuildResult result = gmp::AssemblyGeometryService::build(
      assembly_instances_, &physical_group_service_);
  for (const QString& line : result.log_lines) {
    append_log(line);
  }
  if (!result.ok) {
    if (error) {
    ...
    append_log("Assembly build failed: " + result.error);
    external_model_name_.clear();
    model_loaded_ = false;
    return false;
  }

  // 重登记 Part 来源到模型选择器（UI）。
  QList<QPair<QString, QString>> part_sources;
  if (model_selector_) {
  ...
  if (model_selector_) {
    ...
    for (const auto& entry : part_sources) {
      model_selector_->addItem(entry.first, entry.first);
      model_selector_->setItemData(model_selector_->count() - 1, entry.second,
                                   Qt::UserRole + 1);
    }
  }
...
void GmshPanel::on_generate() {
  ...
  try {
    ...
    // TASK-V02-040：生成执行下沉到 GmshMesher（准备/收尾两段）；
    // 面板保留控件读取、进度窗/进程等待循环与信号发射。
    gmp::MeshJobSpec spec;
    spec.size_x = size_x_->value();
    spec.size_y = size_y_->value();
    spec.size_z = size_z_->value();
    spec.mesh_size = mesh_size_->value();
    spec.elem_order = elem_order_->currentData().toInt();
    spec.msh_version = msh_version_->currentData().toInt();
    spec.high_order_opt =
        high_order_opt_ ? high_order_opt_->currentData().toInt() : 0;
    spec.algo2d = algo2d_ ? algo2d_->currentData().toInt() : 2;
    spec.algo3d = algo3d_ ? algo3d_->currentData().toInt() : 1;
    spec.smoothing = smoothing_ ? smoothing_->value() : 0;
    spec.optimize = optimize_ && optimize_->isChecked();
    spec.topology_mode =
        mesh_topology_mode_ ? mesh_topology_mode_->currentData().toInt() : 0;
    spec.configured_dim =
        mesh_dim_ ? mesh_dim_->currentData().toInt() : -1;
    spec.use_sample_box = use_sample_box_ && use_sample_box_->isChecked();
    spec.external_model_name = external_model_name_;
    spec.output_path = output_path_->text();
    spec.chinese_ui =
    ...
    const qlonglong configured_max =
        qEnvironmentVariable("GMP_MESH_MAX_ESTIMATED_ELEMENTS")
            .toLongLong(&max_ok);
    spec.max_estimated_elements =
        max_ok && configured_max > 0 ? static_cast<double>(configured_max)
                                     : 2000000.0;

    const auto log = [this](const QString& line) { append_log(line); };
    gmp::MeshJobPlan plan = gmp::GmshMesher::prepare(spec, log);
    if (plan.box_created) {
      // 示例盒兜底：服务建好几何，面板同步控件显示。
      model_loaded_ = true;
    ...
    const QString generated_path = plan.generated_path;
    ...
    const double estimated_insertions =
        std::max(1.0, plan.estimated_elements / 8.0);
    ...
    mesh_process.start(plan.gmsh_executable, plan.process_args);
    ...
    // 收尾（校验/落盘/回读/清单）由服务完成；mid_hook 中按既有顺序
    // 发射组清单与 mesh_written 并刷新面板列表。
    const auto mid_hook = [this, &spec](const gmp::MeshJobOutcome& o) {
      emit boundary_groups(o.boundary_names);
      emit volume_groups(o.volume_names);
      append_log("Mesh written: " + spec.output_path);
      emit mesh_written(spec.output_path);
      append_log(QString("Nodes: %1, Elements: %2")
                     .arg(o.node_count)
                     .arg(o.element_count));
      update_entity_summary();
      update_entity_list();
    ...
    const gmp::MeshJobOutcome outcome =
        gmp::GmshMesher::finalize(std::move(plan), spec, log, mid_hook);
    if (!outcome.manifest.isEmpty()) {
      emit mesh_manifest(outcome.manifest);
    }
    success = true;
    if (!plan.structured_fallback_reason.isEmpty()) {
      completion_message =
          gmp::l10n::current_language() == gmp::l10n::Language::Chinese
              ? QString::fromUtf8(
                    ...
                    .arg(plan.structured_fallback_reason)
    ...
    // 规模预检/严格结构化拒绝：消息由服务生成，对话框分支语义不变。
    completion_message = ex.kind() == gmp::MeshJobError::Kind::Generic
                             ? QString("Gmsh error: %1").arg(ex.message())
                             : ex.message();
    append_log(completion_message);
    if (ex.kind() != gmp::MeshJobError::Kind::Generic &&
        !qEnvironmentVariableIsSet("GMP_SCREENSHOT_DIR")) {
      QMessageBox::warning(
          this,
          ex.kind() == gmp::MeshJobError::Kind::StructuredMode
              ? (gmp::l10n::current_language() ==
                         gmp::l10n::Language::Chinese
